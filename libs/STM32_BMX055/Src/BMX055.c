...
bool BMX055_init(BMX055_Handle *bmx055) {
	...
	case BMX055_ACC_RANGE_16:
		// 16g
		bmx055->acc_rescale = 0.00781;
		break;
	case BMX055_ACC_RANGE_8:
		// 8g
		bmx055->acc_rescale = 0.00391;
		break;
	case BMX055_ACC_RANGE_4:
		// 4g
		bmx055->acc_rescale = 0.00195;
		break;
	case BMX055_ACC_RANGE_2:
		// 2g
		bmx055->acc_rescale = 0.00091;
		break;
	}
	...
	// Note: +-2000 deg/s is range of 4000 deg/s. Multiplying this by two yeild the correct
	// result for some reason...
	switch (bmx055->gyro_range) {
	case BMX055_GYRO_RANGE_16_4:
		// 2000
		bmx055->gyro_rescale = 0.061;
		break;
	case BMX055_GYRO_RANGE_32_8:
		// 1000
		bmx055->gyro_rescale = 0.0305;
		break;
	case BMX055_GYRO_RANGE_65_6:
		// 500
		bmx055->gyro_rescale = 0.0153;
		break;
	case BMX055_GYRO_RANGE_131_2:
		// 250
		bmx055->gyro_rescale = 0.0076;
		break;
	case BMX055_GYRO_RANGE_262_4:
		// 125
		bmx055->gyro_rescale = 0.0038;
		break;
	}
...
bool BMX055_searchDevice(BMX055_Handle *bmx055) {
	...
	BMX055_readSPI(bmx055, bmx055->gyro_CS_port, bmx055->gyro_CS_pin, BMX055_WHO_AM_I_REG, &gyro_device, 1);
	BMX055_readSPI(bmx055, bmx055->mag_CS_port, bmx055->mag_CS_pin, BMX055_MAG_REP_CHIP_ID, &mag_device, 1);
	bmx055->acc_good = acc_device == BMX055_ACC_DEVICE;
	bmx055->gyro_good = gyro_device == BMX055_GYRO_DEVICE;
	bmx055->mag_good = mag_device == BMX055_MAG_DEVICE;

	if (bmx055->acc_good && bmx055->gyro_good && bmx055->mag_good) {
		return true;
	} else {
		return false;
	}
...
void BMX055_readGyro(BMX055_Handle *bmx055, float *gyro) {
	...
	gyro[0] = gyro_read[0] * bmx055->gyro_rescale * M_PI / 180;

	gyro_read[1] = ((gyro_data[3] << 8) + gyro_data[2]);
	if (gyro_read[1] > 32767) {
		gyro_read[1] -= 65536;
	}
	gyro[1] = gyro_read[1] * bmx055->gyro_rescale * M_PI / 180;
...
void BMX055_magCalibration(BMX055_Handle *bmx055) {
	uint32_t num_samples = 3000;
	float sample[3];
	float max_arr[] = { 0, 0, 0 };
	float min_arr[] = { 65535, 65535, 65535 };

	for (int i = 0; i < 3000; i++) {
		// Take 3000 samples at a rate of 100 samples/second and store in array
		BMX055_readCompensatedMag(bmx055, sample);
		for (int j = 0; j < 3; j++) {
			if (sample[j] > max_arr[j]) {
				max_arr[j] = sample[j];
			} else if (sample[j] < min_arr[j]) {
				min_arr[j] = sample[j];
			}
		}
		osDelay(10);
	}
	float x_offset = (max_arr[0] - min_arr[0]) / 2;
	float y_offset = (max_arr[1] - min_arr[1]) / 2;
	float z_offset = (max_arr[2] - min_arr[2]) / 2;
