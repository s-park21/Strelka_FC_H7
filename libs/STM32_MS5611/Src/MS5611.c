...
uint32_t MS5611_readRawTemperature(MS5611_Handle *ms5611) {
	...
	uint32_t data;
	if (MS5611_readRegister24(ms5611, MS5611_CMD_ADC_READ, &data)) {
...
uint8_t MS5611_readRegister16(MS5611_Handle *ms5611, uint8_t register_addr, uint16_t *data) {
...
uint8_t MS5611_readRegister24(MS5611_Handle *ms5611, uint8_t register_addr, uint32_t *data) {
	uint8_t read_data[4];
...
uint8_t MS5611_readRegister(MS5611_Handle *ms5611, uint8_t register_addr, uint8_t *data) {
	...
	HAL_GPIO_WritePin(ms5611->baro_CS_port, ms5611->baro_CS_pin, GPIO_PIN_RESET);
	HAL_StatusTypeDef result = HAL_SPI_TransmitReceive(ms5611->hspi, &register_addr, &read_data[0], sizeof(read_data), 1000);
	HAL_GPIO_WritePin(ms5611->baro_CS_port, ms5611->baro_CS_pin, GPIO_PIN_SET);
	if (result == HAL_OK) {
		*data = read_data[1];
		return 0;
